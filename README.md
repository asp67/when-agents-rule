...
**Action set:** `train_unit` · `research_tech` · `upgrade_age` · `build_structure` · `build_wonder` · `assign_workers` · `repair_building` · `explore` · `move_units` · `attack_target` · `delete_unit` · `destroy_building` · `wait`. Villagers are a unit type like any other — `train_unit` with `unitType: "worker"`, hosted at the Town Center.
