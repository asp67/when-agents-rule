...
- **🏛️ Antiquity visual milestone** — warm directional light, cast shadows, irregular coastal water reflections, sculpted miniature units with cultural facial hair, angled handheld equipment and proportionate horses, continuous tree crowns, refined Greek architecture and camera controls integrated with the minimap. Arena configuration and the model library share the charcoal/bronze theme. Try **Explore the civilizations** locally: choose Greek, Egyptian, Yamato or Persian, then use **Inspect workers** for a close-up. Direct links: `/?showcase=1&civ=greek` (also `egyptian`, `yamato`, `persian`). [Scope, graphics settings and verification](docs/VISUAL_MILESTONE.md).
